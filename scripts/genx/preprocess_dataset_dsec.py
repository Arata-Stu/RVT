...
def prophesee_bbox_filter(labels: np.ndarray, dataset_type: str) -> np.ndarray:
    ...
    if dataset_type == 'gen1':
        # The gen1 dataset has a resolution of 304x240
        min_box_diag = 30
        min_box_side = 10
    elif dataset_type == 'gen4':
        # The gen4 dataset has a resolution of 1280x720
        min_box_diag = 60
        min_box_side = 20
    elif dataset_type == 'dsec':
        # The DSEC dataset has a resolution of 640x480
        min_box_diag = 60
        min_box_side = 20
...
def get_base_delta_ts_for_labels_us(unique_label_ts_us: np.ndarray, dataset_type: str = 'gen1') -> int:
    ...
    assert dataset_type == 'gen4' or dataset_type == 'dsec'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ds', '--dataset', default='gen1', help='gen1 or gen4 or dsec')
    parser.add_argument('-np', '--num_processes', type=int, default=1, help="Num processes to run in parallel")
    ...
    if config.event_window_extraction.method == AggregationType.COUNT:
        # イベント数固定の場合
        ev_repr_num_events = config.event_window_extraction.value  # 例: yaml で "value: 20000" など
        # イベント表現生成のステップ幅(ミリ秒)
    ...
    # -------------------------------------------------------------
    # ここから: dataset 種別ごとのシーケンス探索 & train/val/test 準備
    # -------------------------------------------------------------
    seq_data_list = []

    if dataset in ("gen1", "gen4"):
        # すでに train/val/test が分かれているケース (従来通り)
        ...
        for split in ['train', 'val', 'test']:
            ...
            for sequence_dir in split_dir.iterdir():
                ...
                sequence_data = {
                    ...
                    DataKeys.SplitType: split_name_2_type[split],  # TRAIN/VAL/TEST
                }
                seq_data_list.append(sequence_data)

    elif dataset == "dsec":
        # DSEC はもともと train/val/test に分かれていないので、
        # dataset_input_path 配下のシーケンスをすべて探索してランダム分割する。
        all_sequences = []
        for sequence_dir in dataset_input_path.iterdir():
            if not sequence_dir.is_dir():
                continue
        ...
        # train
        for sequence_dir in train_seq_dirs:
            dir_name = sequence_dir.name
            npy_file = sequence_dir / "object_detections" / "left" / "tracks.npy"
            h5f_path = sequence_dir / "events" / "left" / "events.h5"

            out_seq_path = train_out_dir / dir_name
            out_labels_path = out_seq_path / 'labels_v2'
            out_labels_path.mkdir(parents=True, exist_ok=True)
            ...
            sequence_data = {
                ...
                DataKeys.OutLabelDir: out_labels_path,
                DataKeys.OutEvReprDir: out_ev_repr_path,
                DataKeys.SplitType: SplitType.TRAIN,
            }
            seq_data_list.append(sequence_data)

        # val
        for sequence_dir in val_seq_dirs:
            dir_name = sequence_dir.name
            npy_file = sequence_dir / "object_detections" / "left" / "tracks.npy"
            h5f_path = sequence_dir / "events" / "left" / "events.h5"

            out_seq_path = val_out_dir / dir_name
            out_labels_path = out_seq_path / 'labels_v2'
            out_labels_path.mkdir(parents=True, exist_ok=True)
            ...
            sequence_data = {
                ...
                DataKeys.OutLabelDir: out_labels_path,
                DataKeys.OutEvReprDir: out_ev_repr_path,
                DataKeys.SplitType: SplitType.VAL,
            }
            seq_data_list.append(sequence_data)

        # test
        for sequence_dir in test_seq_dirs:
            dir_name = sequence_dir.name
            npy_file = sequence_dir / "object_detections" / "left" / "tracks.npy"
            h5f_path = sequence_dir / "events" / "left" / "events.h5"

            out_seq_path = test_out_dir / dir_name
            out_labels_path = out_seq_path / 'labels_v2'
            out_labels_path.mkdir(parents=True, exist_ok=True)
            ...
            sequence_data = {
                ...
                DataKeys.OutLabelDir: out_labels_path,
                DataKeys.OutEvReprDir: out_ev_repr_path,
                DataKeys.SplitType: SplitType.TEST,
            }
            seq_data_list.append(sequence_data)
    ...
    # -------------------------------------------------------------
    # ここまでで、seq_data_list に (入力np/h5, 出力先ディレクトリ, split種類) 等がすべて格納される。
    # -------------------------------------------------------------
